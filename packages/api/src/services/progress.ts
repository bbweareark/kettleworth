...
import { e1rmSeries, volumeByMuscle, tonnage, weeklyStreak, bestE1RM, isoWeek, type SetRecord } from "@kettleworth/core";

export async function getProgress(userId: string) {
  const since = new Date(Date.now() - 120 * 86400000).toISOString().slice(0, 10);
  const rows = await db().select({ exerciseId: exerciseInstance.exerciseId, loggedSets: exerciseInstance.loggedSets, date: trainingSession.scheduledOn, name: exercise.name, primary: exercise.primaryMuscles }).from(exerciseInstance).innerJoin(trainingSession, eq(exerciseInstance.sessionId, trainingSession.id)).innerJoin(exercise, eq(exerciseInstance.exerciseId, exercise.id)).where(and(eq(trainingSession.userId, userId), eq(trainingSession.status, "completed"), gte(trainingSession.scheduledOn, since)));
  const records: SetRecord[] = [];
  const names: Record<string, string> = {};
  for (const r of rows) { names[r.exerciseId] = r.name; for (const s of r.loggedSets) records.push({ exerciseId: r.exerciseId, date: r.date, set: s, primaryMuscles: r.primary }); }
  ...
  for (const r of records) if (r.set.completed) weeklyTonnage[isoWeek(r.date)] = (weeklyTonnage[isoWeek(r.date)] ?? 0) + (r.set.weightKg ?? 0) * (r.set.reps ?? 0);
  return {
