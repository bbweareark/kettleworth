...
export function MealPlanView({ initial, recipes, targets }: { initial: { weekStartsOn: string; plan: WeeklyMealPlan; coachNote: string | null }; recipes: Record<string, Recipe>; targets: NutritionTargets }) {
  ...
  return (
    <Card><CardContent className="space-y-4">
      ...
      {state.coachNote ? <p className="rounded-lg bg-surface-2 p-3 text-sm text-fg-muted">{state.coachNote}</p> : null}
      ...
      <ul className="space-y-2">{items.map((it) => { const r = recipes[it.recipeId]; if (!r) return null; return (
        <li key={`${it.day}-${it.slot}-${it.recipeId}`} className="flex items-center gap-3 rounded-xl border border-border bg-surface p-3">
          <div className="min-w-0 flex-1"><div className="eyebrow capitalize">{it.slot.replace("_", " ")}</div><button type="button" onClick={() => setOpen(r)} className="truncate text-left font-medium hover:text-ember">{r.name}</button><div className="text-xs text-fg-subtle">{it.servings !== 1 ? `${it.servings}× serving · ` : ""}{it.macros.calories} kcal · {it.macros.proteinG} g protein · <Clock className="inline size-3" /> {r.prepMinutes} min</div></div>
