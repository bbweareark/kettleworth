    ...
    case "injuries": return (
      <div className="space-y-5">
        <Field label="Injuries or pain" hint="Tap a region, then set how much it limits you.">
          ...
          {(p.injuries ?? []).map((inj) => (<div key={inj.region} className="mt-3 flex flex-wrap items-center gap-3 rounded-lg bg-surface-2 p-3"><span className="w-28 text-sm font-medium">{REGIONS.find((r) => r.v === inj.region)?.l}</span><Segmented value={inj.severity} onChange={(v) => set({ injuries: p.injuries!.map((x) => (x.region === inj.region ? { ...x, severity: v } : x)) })} options={[{ value: "mild", label: "Mild" }, { value: "moderate", label: "Moderate" }, { value: "severe", label: "Severe" }]} label={`${inj.region} severity`} /><Input className="h-9 flex-1 text-sm" placeholder="Note (optional)" value={inj.note ?? ""} maxLength={300} onChange={(e) => set({ injuries: p.injuries!.map((x) => (x.region === inj.region ? { ...x, note: e.target.value } : x)) })} /></div>))}
        </Field>
