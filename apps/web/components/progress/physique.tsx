...
export function PhysiqueCharts({ timeline, trends, unit }: { timeline: Point[]; trends: { bodyFat: Trend; leanMass: Trend; weight: Trend; waist: Trend }; unit: "kg" | "lb" }) {
  ...
  const data = timeline.map((t) => ({ date: t.date.slice(5), bf: t.bodyFatPct, lo: t.bodyFatLow, hi: t.bodyFatHigh, lean: t.leanMassKg != null ? Math.round(t.leanMassKg * k * 10) / 10 : null, fat: t.fatMassKg != null ? Math.round(t.fatMassKg * k * 10) / 10 : null, weight: t.weightKg != null ? Math.round(t.weightKg * k * 10) / 10 : null }));
  const hasBf = data.some((d) => d.bf != null), hasLean = data.some((d) => d.lean != null);
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-2">
      <div>
        <div className="mb-1 flex items-center justify-between"><h3 className="font-display text-base font-semibold">Body composition</h3><span className="text-xs text-fg-subtle">{trends.leanMass.sentence}</span></div>
        {hasLean ? (<div className="h-56"><ResponsiveContainer><AreaChart data={data} margin={{ left: -12, right: 8, top: 8 }}><defs><linearGradient id="lean" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--color-signal)" stopOpacity={0.5} /><stop offset="100%" stopColor="var(--color-signal)" stopOpacity={0} /></linearGradient><linearGradient id="fat" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--color-amber)" stopOpacity={0.4} /><stop offset="100%" stopColor="var(--color-amber)" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="var(--color-border)" vertical={false} /><XAxis dataKey="date" {...axis} /><YAxis {...axis} /><Tooltip {...tip} formatter={(v, n) => [`${v} ${unit}`, n === "lean" ? "Lean mass" : "Fat mass"]} /><Area type="monotone" dataKey="lean" stackId="1" stroke="var(--color-signal)" fill="url(#lean)" connectNulls /><Area type="monotone" dataKey="fat" stackId="1" stroke="var(--color-amber)" fill="url(#fat)" connectNulls /></AreaChart></ResponsiveContainer></div>) : <p className="text-sm text-fg-subtle">Needs a weigh-in plus a body check to estimate lean and fat mass.</p>}
      </div>
      <div>
        <div className="mb-1 flex items-center justify-between"><h3 className="font-display text-base font-semibold">Body fat estimate</h3><span className="text-xs text-fg-subtle">{trends.bodyFat.sentence}</span></div>
        {hasBf ? (<div className="h-56"><ResponsiveContainer><LineChart data={data} margin={{ left: -12, right: 8, top: 8 }}><CartesianGrid stroke="var(--color-border)" vertical={false} /><XAxis dataKey="date" {...axis} /><YAxis {...axis} domain={["dataMin - 3", "dataMax + 3"]} /><Tooltip {...tip} formatter={(v, n) => [`${v}%`, n === "bf" ? "Estimate" : n === "lo" ? "Low" : "High"]} /><Line type="monotone" dataKey="hi" stroke="var(--color-border-strong)" strokeDasharray="3 3" dot={false} connectNulls /><Line type="monotone" dataKey="lo" stroke="var(--color-border-strong)" strokeDasharray="3 3" dot={false} connectNulls /><Line type="monotone" dataKey="bf" stroke="var(--color-ember)" strokeWidth={2.5} dot={{ r: 4 }} connectNulls /></LineChart></ResponsiveContainer></div>) : <p className="text-sm text-fg-subtle">Take a body check to start the estimate line. Dashed lines show the range the coach could see.</p>}
