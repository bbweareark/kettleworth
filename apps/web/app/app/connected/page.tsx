...
export default async function Connected({ searchParams }: { searchParams: Promise<{ connected?: string; error?: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-3xl p-8 ring-1 ring-white/[0.06]"><img src="/art/connected.jpg" alt="" aria-hidden className="absolute inset-0 -z-20 size-full object-cover opacity-80" /><div aria-hidden className="absolute inset-0 -z-10 bg-[linear-gradient(90deg,var(--color-bg)_0%,color-mix(in_oklch,var(--color-bg)_75%,transparent)_55%,color-mix(in_oklch,var(--color-bg)_30%,transparent)_100%)]" /><p className="eyebrow">Connected apps</p><h1 className="font-display text-4xl font-semibold tracking-tightest md:text-5xl">Let recovery shape the plan.</h1><p className="mt-2 max-w-xl text-fg-muted">Sleep, HRV and readiness ease or push each session. You choose the data types; one tap disconnects and deletes.</p></div>
