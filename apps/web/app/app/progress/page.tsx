...
import { getProfile, getProgress, listPhotos, aiAvailable, physiqueProgress, liftsOverview } from "@kettleworth/api";
import { LiftsList } from "@/components/lifts/lifts-list";
...
export default async function Progress() {
  ...
  const [p, photos, phys, lifts] = await Promise.all([getProgress(user.id), listPhotos(user.id), physiqueProgress(user.id), liftsOverview(user.id)]);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><div className="eyebrow">Your lifts</div><h2 className="font-display text-2xl font-semibold tracking-tighter">Every lift, and where it stands.</h2></div><p className="max-w-sm text-xs text-fg-subtle">Last session, heaviest set and best estimated max for each exercise. Tap one for its full history.</p></div>
        <LiftsList lifts={JSON.parse(JSON.stringify(lifts))} units={rec.profile.units} />
      </section>
      <section className="space-y-3">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card><CardContent><h2 className="mb-3 flex items-center gap-2 font-display text-lg font-semibold"><Flame className="size-4 text-ember" /> Recent sessions</h2>{p.recentSessions.length ? <ul className="divide-y divide-border">{p.recentSessions.map((s, i) => <li key={i} className="flex items-center justify-between py-2 text-sm"><span>{s.name}</span><span className="flex items-center gap-2 text-fg-muted">{s.sessionRpe ? <Badge tone="outline">RPE {s.sessionRpe}</Badge> : null}{dayMonth(s.scheduledOn)}</span></li>)}</ul> : <p className="text-sm text-fg-subtle">No sessions completed yet.</p>}</CardContent></Card>
      </div>
    </div>
  );
}
