...
export default async function Nutrition() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-6 rounded-3xl bg-surface/40 p-6 ring-1 ring-white/[0.04] md:grid-cols-[auto_1fr] md:items-center">
        ...
        <div className="grid grid-cols-3 gap-4 md:justify-self-end">
          {[["Train day", t.trainingDayCalories, "kcal"], ["Rest day", t.restDayCalories, "kcal"], ["Water", (t.waterMl / 1000).toFixed(1), "L"]].map(([l, v, u]) => (<div key={l as string}><div className="text-2xs uppercase tracking-[0.16em] text-fg-subtle">{l as string}</div><div className="font-display text-3xl font-semibold tabular tracking-tightest">{v as number}<span className="text-sm text-fg-subtle"> {u as string}</span></div></div>))}
